...
export const Form = () => {
  ...
  const onSubmitHandler = (e: React.SyntheticEvent) => {
    ...
    if (!isPasswordValid) {
      setMessage(
        'Password at least 8 minimum character, One uppercase & Lowercase letter'
      );
      setStatus('error');
      console.log('lol');
      toastRef.current && toastRef.current.showToast();
  ...
  return (
    ...
    >
      ...
      <button
        type='submit'
        className='mt-32 bg-blue-500 text-white px-3 py-3 rounded-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-blue-400 cursor-pointer flex justify-center items-center'
      >
        <img
          src='/assets/loading.svg'
          alt='loading'
          className={`w-[10px] h-[10px] animate-spin mr-2 ${
            isFetching ? null : 'hidden'
          }`}
        />
        Register
      </button>
      ...
      <img
        src='/assets/registerAssets2.png'
        alt=''
        className='absolute -top-[170px] left-[20px] hidden lg:flex'
      />
      <img
        src='/assets/registerAssets3.png'
        alt=''
        className='absolute -top-40 left-16 hidden lg:flex'
      />
    </form>
  );
};
